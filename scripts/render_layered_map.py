...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--occupancy',type=Path,required=True);ap.add_argument('--topology',type=Path,required=True);ap.add_argument('--output',type=Path,required=True);ap.add_argument('--route',type=Path);ap.add_argument('--scale',type=int,default=4);a=ap.parse_args()
 ...
 for i,n in enumerate(topo['nodes']):
  x,y=map(int,n['pixel']); c=(242,145,35) if n['node_id'] in route else (30,125,92);d.ellipse((x-3,y-3,x+3,y+3),fill=c,outline='white');d.text((x+4,y-4),n['node_id'].replace('topo_','T'),fill=(20,20,20))
 im.resize((w*a.scale,h*a.scale),Image.Resampling.NEAREST).save(a.output)
 meta={'schema_version':'0.1','type':'layered_space_topology_visualization','spatial_layer':str(a.occupancy.resolve()),'topology_layer':str(a.topology.resolve()),'route':str(a.route.resolve()) if a.route else None,'obstacle_outline':'derived from occupied/free/unknown boundaries','graph_edges_source':'topology JSON only','nodes':len(topo['nodes']),'edges':len(topo.get('edges',[])),'route_nodes':sorted(route)}
