...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--patrol',type=Path,required=True);ap.add_argument('--rgb-dir',type=Path,required=True);ap.add_argument('--depth-dir',type=Path,required=True);ap.add_argument('--depth-suffix',default='.npy');ap.add_argument('--depth-model',default='sensor_depth');ap.add_argument('--output',type=Path,required=True);ap.add_argument('--width',type=int,default=640);ap.add_argument('--height',type=int,default=480);ap.add_argument('--hfov',type=float,default=90.0);a=ap.parse_args()
 p=json.loads(a.patrol.read_text());focal=.5*a.width/math.tan(math.radians(a.hfov)/2);frames=[]
 for f in p['frames']:
  stem=f['frame_id'];rgb=a.rgb_dir/f'{stem}.png';depth=a.depth_dir/f'{stem}{a.depth_suffix}'
 ...
 out={'schema_version':'0.2','type':'robot_patrol_rgbd_manifest','world_role':'A_sensor_simulation','source_patrol':str(a.patrol.resolve()),'hidden_occupancy_consumed':False,'hidden_labels_consumed':False,'depth_model':a.depth_model,'frames':frames};a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(json.dumps(out,ensure_ascii=False,indent=2)+'\n');print(json.dumps({'frames':len(frames),'output':str(a.output),'depth_model':a.depth_model},ensure_ascii=False))
