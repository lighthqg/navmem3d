...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--manifest',type=Path,required=True);ap.add_argument('--output-dir',type=Path,required=True);ap.add_argument('--scale',type=float,default=.05);ap.add_argument('--sample',type=int,default=6);ap.add_argument('--scanline-rows',default='',help='comma-separated depth rows for a 2D LiDAR-like scan');ap.add_argument('--max-depth',type=float,default=12);ap.add_argument('--robot-radius',type=float,default=.2);a=ap.parse_args()
 ...
 meta={'schema_version':'0.3','type':'robot_observed_occupancy','source_manifest':str(a.manifest.resolve()),'hidden_occupancy_consumed':False,'hidden_labels_consumed':False,'state_encoding':{'occupied':0,'unknown':127,'free':255},'scale_m':a.scale,'origin_xy':lo.tolist(),'width':w,'height':h,'robot_radius_m':a.robot_radius,'depth_sample_stride':a.sample,'scanline_rows':a.scanline_rows or None,'traversed_footprint_carved':True,'observed_free_cells':int(np.count_nonzero(inflated==255)),'observed_occupied_cells':int(np.count_nonzero(inflated==0)),'unknown_cells':int(np.count_nonzero(inflated==127))}
